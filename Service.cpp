...
bool Service::init()
{
    ...
    //bind(servicefd, (struct sockaddr *)&service_addr, sizeof service_addr);
...
void Service::run()
{
    while (servicefd>0)
    {
        ...
        {
            if(clientfds.count(events[i].data.fd) && events[i].events& EPOLLOUT)
            {
            ...
            if(clientfds.count(events[i].data.fd)&& events[i].events & EPOLLIN)
            {
                ...
                while ((n = read(events[i].data.fd, messagehead_r, 6))>0)
                {
                    ...
                    if (version==0 && type == 5 && users == 0&& datalength ==0)  // 第一次连接
                    {
            ...
            if (servicefd == events[i].data.fd && events[i].events& EPOLLIN)  // 新的连接
            {
                int conn_sock;
                sockaddr_in remote;
                socklen_t remotelen;
